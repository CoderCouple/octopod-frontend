...
import { connectOutlook } from "@/api/email-sequence";
...
export default function MicrosoftOauthCallbackPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      if (errorParam) {
        setStatus("error");
        setMessage(`Microsoft returned: ${errorDesc || errorParam}`);
        return;
      }
      if (!code) {
        setStatus("error");
      ...
      try {
        const mailbox = await connectOutlook({ auth_code: code });
        if (cancelled) return;
        setStatus("ok");
        ...
        setStatus("error");
        setMessage(
          e instanceof Error ? e.message : "Failed to connect Outlook mailbox"
        );
      }
    }
    run();
  ...
  }, [code, errorParam, errorDesc, state, router]);
